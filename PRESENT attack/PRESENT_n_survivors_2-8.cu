...
/* ─── 6-round PRESENT, stores full 64-bit state ───────────────────────── */
__global__ void PRESENT6_coset_kernel(uint64_t       baseP,
                                      const uint64_t rk[32],  // first 9 used
                                      const uint64_t * __restrict__ p0,
                                      const uint64_t * __restrict__ p1,
                                      const uint64_t * __restrict__ p2,
                                       ...
                                       uint64_t*        __restrict__ ct_out)
{
    ...
    ct_out[gid] = state;  // store full 64-bit state
	
}

// -----------------------------------------------------------------------------
// host wrapper – fills     h_ct_coset[4096]  with the 6-round ciphertexts
// -----------------------------------------------------------------------------
void encrypt_6round_coset(uint64_t baseP, uint64_t key80_out[2])          // ← returns key
{
    /* -------- choose reproducible 80-bit master-key ----------------------- */
    
    uint64_t h_ct_coset[COS_SZ];

    /* -------- pick random coset base (still reproducible) ----------------- */
    printf("Coset base: 0x%016llx\n", (unsigned long long)baseP);

    /* -------- device allocations ------------------------------------------ */
    uint64_t *d_ct = nullptr;
    uint64_t *d_rk = nullptr;
    cudaMalloc(&d_ct, COS_SZ* sizeof(uint64_t));  
    cudaMalloc(&d_rk, 32     * sizeof(uint64_t));
    cudaMemcpy(d_rk, round_key, 32*sizeof(uint64_t), cudaMemcpyHostToDevice);
    ...
    /* -------- launch kernel ------------------------------------------------ */
    PRESENT6_coset_kernel<<<R8_BLOCKS,R8_THREAD>>>(baseP, d_rk,
                                                   d_p0,d_p1,d_p2,d_p3,
                                                   d_p4,d_p5,d_p6,d_p7,
                                                   d_ct);
    cudaDeviceSynchronize();
    ...
    /* -------- fetch results + clean-up ------------------------------------ */
    cudaMemcpy(h_ct_coset, d_ct, COS_SZ*sizeof(uint64_t), cudaMemcpyDeviceToHost);


    uint32_t cnt[4] = {0,0,0,0};
    for (int i = 0; i < COS_SZ; ++i) {
        uint64_t c = h_ct_coset[i];
  
        
        uint8_t nib = c & 0xF;          // last 4 bits of ciphertext
        cnt[0] +=  nib       & 1u;
        cnt[1] += (nib >> 1) & 1u;
        cnt[2] += (nib >> 2) & 1u;
        cnt[3] +=  (nib >> 3) & 1u;
    ...
    printf("6-round integral test: %d %d %d %d\n", cnt[0], cnt[1], cnt[2], cnt[3]);
...
/* run one full attack, return (#survivors, seed) */
struct RunResult { uint32_t survivors; uint64_t seed; };
...
RunResult run_once(uint64_t seed, std::ofstream& log)
{
    ...
    /* optional sanity: check membership of real key, omitted here */

    /* ---------- write one CSV line -------------------------------------- */
    log << seed << ',' << subs.size() << '\n';
    log.flush();

    return { static_cast<uint32_t>(subs.size()), seed };
}
...
int main(int argc, char* argv[])
{
    ...
    // std::ofstream log("integral_runs.csv",
    ...
    // }
    // std::cout << "\nAll results written to integral_runs.csv\n";





    const char* in_name  = "integral_runs.csv";
    const char* out_name = "integral_runs_with_key.csv";

    std::ifstream in(in_name);
    if (!in) { std::cerr << "cannot open " << in_name << '\n'; return 1; }

    std::ofstream out(out_name);
    /* ---- copy header & add new column ---------------------------- */
    std::string header;
    std::getline(in, header);
    out << header << ",true_partial_key\n";
    ...
    while (std::getline(in, line))
    {
        ...
        std::string seed_str, surv_str;
        std::getline(iss, seed_str, ',');
        std::getline(iss, surv_str, ',');

        uint64_t seed = std::stoull(seed_str, nullptr, 0);

        /* --- regenerate round keys from the seed ----------------- */
        init_rng(seed);
        uint64_t key80[2] = { rand64(), rand64() & 0xFFFF };
        key_schedule(key80);               // fills round_key[]

        uint32_t pkey = true_partial_key();

        /* --- write original line + new column -------------------- */
        out << seed_str << ',' << surv_str << ',' << std::hex
            << std::showbase << pkey << std::dec << '\n';
    }
    std::cout << "output written to " << out_name << '\n';
