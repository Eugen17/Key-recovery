...
void encrypt_8round_coset(CtWord h_ct_coset[COS_SZ],
                          uint64_t key80[2], uint64_t* baseP_out)          // ← returns key
{
...
RunResult run_once(uint64_t seed, std::ofstream& log, uint64_t key80[2])
{
    init_rng(seed);
...
int main(int argc, char* argv[])
{
    ...
    std::ofstream log("integral_runs_one_key.csv",
    ...
    for (int i = 0; i < N; ++i) {
        ...
        RunResult r = run_once(seed, log, key80);
        std::cout << "Run " << i
                  << "  seed=0x" << std::hex << r.seed
                  << "  survivors=" << std::dec << r.survivors << '\n';
    }
    std::cout << "\nAll results written to integral_runs_one_key.csv\n";
    return 0;
}